...
import io.github.masyumero.emextras.common.tile.factory.TileEntityItemStackToItemStackEMExtraFactory;
...
/**
	 * EME扩展版MEK离心机工厂方块实体 — 继承EME的 TileEntityItemStackToItemStackEMExtraFactory。
	 * <br/>
	 ...
	 */
public class TileEntityEMExtraMekCentrifugeFactory extends TileEntityItemStackToItemStackEMExtraFactory
	...
	public TileEntityEMExtraMekCentrifugeFactory(mekanism.api.providers.IBlockProvider blockProvider, BlockPos pos, BlockState state) {
	...
	public CompoundTag getConfigurationData(@NotNull net.minecraft.world.entity.player.Player player) {
		...
		// EME 的 TileEntityEMExtraFactory 继承 Mekanism TileEntityFactory（sorting 与 isSorting() 同源），
		// 仍用 accessor 显式写入保证跨版本行为一致
		data.putBoolean("sorting", ((TileEntityEMExtraFactoryAccessor) this).productivebeesgenesis$getSorting());
		return data;
	}
