...
- **编译依赖本地化**：移除对个人机器路径（`G:\versions\2\mods`、`C:\Users\XBX\...`）的硬编码引用，全部 13 个 1.20.1 Forge 编译依赖统一放入项目 `libs/`（不入库），本地一条命令获取：`scripts/fetch-libs.ps1`（Windows）或 `scripts/fetch-libs.sh`（Linux/macOS），CI 使用同一脚本，来源固定（CurseMaven / Modrinth）；ProductiveLib 1.20.1-0.0.4 从 Productive Bees jar 内嵌 jarjar 提取，与主线 1.21.1 CI 的做法一致。JEI 使用官方 15.48.0.185 替代原 JEIunofficial 15.48.0.182（API 兼容）。`libs/` 缺失时构建会给出明确提示。
